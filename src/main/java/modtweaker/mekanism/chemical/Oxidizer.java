...
public class Oxidizer {
	public static class Add extends TweakerBaseFunction {
		...
		private Add() {
			super("mekanism.chemical.oxidizer.addRecipe");
		}
		...
		public void perform(TweakerValue... args) {
			if (canContinue(2, args)) {
				...
				Tweaker.apply(new MekanismAddRecipeAction(Recipe.CHEMICAL_OXIDIZER, input, output));
	...
	public static class Remove extends TweakerMekanismBaseRemove {
		...
		private Remove() {
			super("mekanism.chemical.oxidizer.removeRecipe", Recipe.CHEMICAL_OXIDIZER);
		}
	}
}
