...
public class TweakerHelper {
	...
	public static boolean getBoolean() {
		boolean i = notNull(arguments[index], index).toBool("argument " + index + " must be a boolean").get();
		index++;
		return i;
	}
	...
	public static ItemStack[] getItemOre() {
		TweakerItemStack stack = notNull(arguments[index], index).asItemStack();
		ItemStack[] ret = stack != null? new ItemStack[] { stack.get() }: (ItemStack[]) OreDictionary.getOres(getString()).toArray();
		if(ret != null) {
			index++;
		}
