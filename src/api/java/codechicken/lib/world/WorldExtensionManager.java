...
public class WorldExtensionManager {
    public static class WorldExtensionEventHandler {
        ...
        public void onChunkDataSave(ChunkDataEvent.Save event) {
            try {
                ...
                if (!event.getChunk().isChunkLoaded) removeChunk(event.world, event.getChunk());
            } catch (Exception e) {}
        }
        ...
        public void onWorldSave(WorldEvent.Save event) {
            if (worldMap.containsKey(event.world)) for (WorldExtension extension : worldMap.get(event.world))
                extension.save();
        }
        ...
        public void clientTick(TickEvent.WorldTickEvent event) {
            if (event.phase == TickEvent.Phase.START) preTick(event.world);
            else postTick(event.world);
        }
    }
    ...
    private static void createChunkExtension(World world, Chunk chunk) {
        ...
        for (int i = 0; i < extensionIntialisers.size(); i++)
            if (!extensions[i].containsChunk(chunk)) extensions[i].addChunk(extensionIntialisers.get(i).createChunkExtension(chunk, extensions[i]));
    }
