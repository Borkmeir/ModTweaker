...
public class BlockExtendedFire extends BlockFire
{
    ...
    public void updateTick(World world, int x, int y, int z, Random rand)
    {
        if(world.getGameRules().getGameRuleBooleanValue("doFireTick"))
        {
            ...
            if(!flag && (world.canLightningStrikeAt(x, y, z) || world.canLightningStrikeAt(x - 1, y, z) || world.canLightningStrikeAt(x + 1, y, z) || world.canLightningStrikeAt(x, y, z - 1) || world.canLightningStrikeAt(x, y, z + 1)))
                world.setBlock(x, y, z, Blocks.fire);
            else
            {
                ...
                if(!flag && !this.func_149847_e(world, x, y, z))
                {
                            ...
                            for(int k1 = y - 1; k1 <= y + 4; ++k1)
                            {
                                if(i1 != x || k1 != y || j1 != z)
                                {
                                    ...
                                    if(i2 > 0)
                                    {
                                        ...
                                        if(j2 > 0 && rand.nextInt(l1) <= j2 && !world.canLightningStrikeAt(i1 - 1, k1, z) && !world.canLightningStrikeAt(i1 + 1, k1, j1) && !world.canLightningStrikeAt(i1, k1, j1 - 1) && !world.canLightningStrikeAt(i1, k1, j1 + 1))
                                        {
    ...
    private boolean func_149847_e(World world, int x, int y, int z)
    {
